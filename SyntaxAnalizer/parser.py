    ...
    def parse(self):
        ...
        while not self.is_at_end():
            ...
            if programm is not None:
                statements.append(programm)
        return statements

#Проверяет тип токена и модифицирует его, если следующий токен является левой скобкой
...
#Проверяет, соответствует ли текущий токен одному из переданных типов, и продвигает текущую позицию, если это так
...
#Потребляет токен определенного типа или вызывает ошибку, если ожидаемый токен не найден
...
#Продвигает текущую позицию в списке токенов и возвращает предыдущий токен
    def advance(self):
...
#Синхронизирует парсер после возникновения ошибки, чтобы продолжить разбор с следующего безопасного места
...
#Пытается разобрать начало и возвращает соответствующий объект. В случае ошибки парсинга синхронизирует парсер и продолжает разбор
    def programm(self):
        try:
...
#Разбирает класс, включая его имя, суперкласс и методы
...
#Определяет тип и вызывает соответствующий метод
    def statement(self):
...
#Обрабатывает код, заканчивающиеся точкой с запятой
...
#Обрабатывает блок кода, состоящий из нескольких инструкций
    def block(self):
        statements = []
...
#Обрабатывает операции присваивания
    def assignment(self):
        expr = self.or_()
...
#Обрабатывает операции равенства
    def equality(self):
...
#брабатывает сложение и вычитание
    def term(self):
        expr = self.factor()
